...
public:
    ...
    void verify(const jwt::decoded_jwt &jwt) {
        ...
        while (!result->m_done) {
            result = verify_async(jwt);
        }
    }

    std::unique_ptr<AsyncStatus> verify_async(const jwt::decoded_jwt &jwt)
    {
    ...
    std::unique_ptr<AsyncStatus> verify_async_continue(std::unique_ptr<AsyncStatus> status)
    {
        if (!status->m_done) {
            ...
            status->m_public_pem = public_pem;
            status->m_algorithm = algorithm;
            if (!status->m_done) {return status;}
        }
        ...
        const jwt::decoded_jwt jwt(status->m_jwt_string);
...
private:
    std::unique_ptr<AsyncStatus> get_public_key_pem(const std::string &issuer, const std::string &kid, std::string &public_pem, std::string &algorithm);
    std::unique_ptr<AsyncStatus> get_public_key_pem_continue(std::unique_ptr<AsyncStatus> status, std::string &public_pem, std::string &algorithm);
    std::unique_ptr<AsyncStatus> get_public_keys_from_web(const std::string &issuer);
    std::unique_ptr<AsyncStatus> get_public_keys_from_web_continue(std::unique_ptr<AsyncStatus> status);
